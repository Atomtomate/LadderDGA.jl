...
χ_m, γ_m = calc_χγ(:m, Γsp, bubble, kG, mP, sP);
χ_d, γ_d = calc_χγ(:d, Γch, bubble, kG, mP, sP);
cs_χm = abs(sum(χ_m))
cs_χd = abs(sum(χ_d))
cs_γm = abs(sum(γ_m))
cs_γd = abs(sum(γ_d))

χ_m_par, γ_m_par = calc_χγ_par(:m, Γsp, kG, mP, sP);
χ_d_par, γ_d_par = calc_χγ_par(:d, Γch, kG, mP, sP);
...
Σ_ladder = calc_Σ(χ_m, γ_m, χ_d, γ_d, χloc_m_sum, λ₀, gLoc_rfft, kG, mP, sP, νmax=sP.n_iν);
initialize_EoM(gLoc_rfft, χloc_m_sum, λ₀, 0:sP.n_iν-5, kG, mP, sP, χ_m = χ_m, γ_m = γ_m,
...
initialize_EoM(gLoc_rfft, χloc_m_sum, λ₀, 0:sP.n_iν-1, kG, mP, sP, 
                χ_m = χ_m, γ_m = γ_m,
                χ_d = χ_d, γ_d = γ_d, force_reinit=true)
...
Σ_ladder_dm_2 = calc_Σ(χ_m, γ_m, χ_d, γ_d, χloc_m_sum, λ₀, gLoc_rfft, kG, mP, sP, νmax=last(axes(res_λdm[2],2))+1, λm=res_λdm[7], λd=res_λdm[end]);
