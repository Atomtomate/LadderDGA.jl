...
struct χ₀T <: MatsubaraFunction{_eltype,3}
    ...
    function χ₀T(
        ...
        νnGrid = -n_iν:n_iν-1
        indices_νω = reshape(
            [(j, i) for i in ωnGrid for j in νnGrid .- trunc(Int64, shift * i / 2)],
            (length(νnGrid), length(ωnGrid)),
        )
...
function update_tail!(χ::χT, new_tail_c::Array{Float64}, ωnGrid::Array{ComplexF64})
    length(new_tail_c) != length(χ.tail_c) && throw(
        ArgumentError(
            "length of new tail coefficient array ($(length(new_tail_c))) must be the same as the old one ($(length(χ.tail_c)))!",
        ),
    )
    length(ωnGrid) != size(χ.data, 2) &&
        throw(ArgumentError("length of ωnGrid ($(length(ωnGrid))) must be the same as for χ ($(size(χ.data,2)))!"))
...
function update_tail!(
    ...
    old_tail_c::Vector{Float64},
    new_tail_c::Vector{Float64},
    ωnGrid::Vector{ComplexF64},
)
    ci::Int = 3

    zero_ind = findfirst(x -> abs(x) < 1e-10, ωnGrid)
    if old_tail_c[ci] != new_tail_c[ci]
        length(new_tail_c) != length(old_tail_c) && throw(
            ArgumentError(
                "length of new tail coefficient array ($(length(new_tail_c))) must be the same as the old one ($(length(χ.tail_c)))!",
            ),
        )
