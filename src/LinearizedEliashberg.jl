            ...
            if freq_inbounds(ωi_ladder, νi_ladder, νpi_ladder, h.sP)
                for (kpi, kp_vec) in enumerate(k_vecs)
                    ...
                    for (ki, k_vec) in enumerate(k_vecs)
                        ...
                        #
                        Γs_ladder1[ki+length(k_vecs)*(νi-1), kpi+length(k_vecs)*(νpi-1)] =
                            -(Fph_ladder_updo[qi, νi_ladder, νpi_ladder, ωi_ladder] .- Phi_ud[νi_pp, νpi_pp, ωi_pp]) * G_mG / (2 * kG.Nk * h.mP.β)
                    end
                end
            end
        end
...
function calc_λmax_linEliashberg_MatrixFree(bubble::χ₀T, χm::χT, χd::χT, γm::γT, γd::γT, h::lDΓAHelper, env; GF = h.gLoc, max_Nk::Int = h.kG.Ns, χm_star_gen = nothing, χd_star_gen = nothing)
    ...
    Fd_loc = F_from_χ(:d, h)
    Gνk_Gmνmk = build_GG(kG, GF[sub_i, :], νnGrid, k_vecs[:])
    qi_access = build_q_access(h.kG, k_vecs[:])
                ...
                if freq_inbounds(ωi_ladder, νi_ladder, νpi_ladder, h.sP)
                    for (kpi, kp_vec) in enumerate(k_vecs)
                        ...
                        for (ki, k_vec) in enumerate(k_vecs)
                            ...
                            res[ki+klen*(νi-1)] += -vec[kpi+klen*(νpi-1)] * (Fph_ladder_updo[qi_minus_k_minus_kp, νi_ladder, νpi_ladder, ωi_ladder] .- Phi_ud[νi_pp, νpi_pp, ωi_pp]) * G_mG
                        end
                    end
                else
