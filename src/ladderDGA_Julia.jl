...
#TODO: write macro, to reduce number of function parameters (modelParams, simParams, qGrid, etc)
...
qIndices, _     = reduce_kGrid.(cut_mirror.((kIndices, kGrid)))
...
@time χsp, χch, 
      χsp_ω, χch_ω, 
      trilexsp, trilexch, 
      usable_sp, usable_ch = calc_χ_trilex(Γsp, Γch, bubble, qMultiplicity, modelParams, simParams, env)
    
println("Found usable intervals for non-local susceptibility of length 
         sp: $(length(usable_sp)) ch: $(length(usable_ch))")
...
if simParams.tail_corrected
    ...
    println("Using rhs for non tail corrected lambda correction: ", real(rhs), " = ", 
             real(χLocch), " + ", real(χLocsp), " - ", real(χch_sum))
end
...
if !simParams.chi_only
    ...
    @time Σ_ladder = calc_DΓA_Σ_fft(χsp, χch, trilexsp, trilexch, bubble, Σ_loc, FUpDo, 
                                  ϵkGrid, qIndices, modelParams, simParams)
