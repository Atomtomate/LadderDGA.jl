...
function calc_bubble(Gνω::GνqT, kGrid::ReducedKGrid, 
        mP::ModelParameters, sP::SimulationParameters)
    res = SharedArray{Complex{Float64},3}(2*sP.n_iω+1, length(kGrid.kMult), 2*sP.n_iν, pids=workers())
    @sync @distributed for ωi in axes(res,1)
        for νi in axes(res,3)
            ωn, νn = OneToIndex_to_Freq(ωi, νi, sP)
            v1 = view(Gνω, νn + sP.fft_offset, :)
            v2 = view(Gνω, νn+ωn + sP.fft_offset, :)
...
function calc_χ_trilex(Γr::SharedArray{Complex{Float64},3}, bubble::SharedArray{Complex{Float64},3}, 
                       kGrid::ReducedKGrid, U::Float64, mP::ModelParameters, sP::SimulationParameters)
    χ = SharedArray{eltype(bubble), 2}((size(bubble)[1:2]...), pids=workers())
    γ = SharedArray{eltype(bubble), 3}((size(bubble)...), pids=workers())
    ...
    @sync @distributed for ωi in ωindices
        ...
        for qi in axes(bubble,2)
            ...
            @inbounds χ[ωi, qi] = sum_freq_full(χ_full, sP.sh_f, mP.β)
            #TODO: absor this loop into sum_freq, partial sum is carried out twice
            for νp in axes(γ, 3)
                @inbounds γ[ωi, qi, νp] = sum_freq_full((@view χ_full[νp,:]), sP.sh_f, 1.0) / (bubble[ωi, qi, νp] * (1.0 + U * χ[ωi, qi]))
            end
    ...
    usable = if fixed_ω
        ...
        find_usable_interval(χ_ω, sum_type=sP.ωsum_type, reduce_range_prct=sP.usable_prct_reduction)
    end
...
function Σ_internal!(tmp::AbstractArray{Float64,3}, ωindices::AbstractArray{Int,1}, bubble::BubbleT, 
                     FUpDo::SharedArray{Complex{Float64},3}, sh_f::T) where T <: SumHelper
    @sync @distributed for ωii in 1:length(ωindices)
        ωi = ωindices[ωii]
        for qi in axes(bubble,2)
            for νi in axes(tmp,3)
                @inbounds tmp[ωii, qi, νi] = real(sum_freq_full(FUpDo[ωi,νi,:] .* bubble[ωi,qi,:], sh_f, 1.0))
            end
        end
    end
end
...
"""
function calc_Σ_ω!(Σ::SharedArray{Complex{Float64},3}, ωindices::AbstractArray{Int,1},
            Q_sp::TQ, Q_ch::TQ,Gνω::GνqT, tmp::SharedArray{Float64,3}, U::Float64, kGrid::ReducedKGrid, 
    ...
    @sync @distributed for ωii in 1:length(ωindices)
        ...
        for (νn,νi) in enumerate(νZero:maxn)
            ...
            Σ[ωii, :, νn] = conv_fft1(kGrid, Kνωq, view(Gνω, (νn-1) + ωn + sP.fft_offset,:))
        end
    end
end

function calc_Σ_dbg(Q_sp::NonLocalQuantities, Q_ch::NonLocalQuantities, bubble::BubbleT,
                Gνω::GνqT, FUpDo::SharedArray{Complex{Float64},3}, kGrid::ReducedKGrid,
                mP::ModelParameters, sP::SimulationParameters)
    ...
    tmp = SharedArray{Float64,3}(length(ωindices), size(bubble,2), size(bubble,3), pids=workers())
    Σ_ladder_ω = SharedArray{Complex{Float64},3}( size(bubble,1), size(bubble,2), sP.n_iν, pids=workers())
    ...
    Σ_ladder_ω = permutedims(Σ_ladder_ω, [1,3,2])
    res = mP.U .* sum_freq(Σ_ladder_ω, [1], sh_b, mP.β)[1,:,:]
...
function calc_Σ(Q_sp::NonLocalQuantities, Q_ch::NonLocalQuantities, bubble::BubbleT,
    ...
    tmp = SharedArray{Float64,3}(length(ωindices), size(bubble,2), size(bubble,3), pids=workers())
    Σ_ladder_ω = SharedArray{Complex{Float64},3}( length(ωindices), size(bubble,2), sP.n_iν, pids=workers())
