...
function calc_bubble_int(Gνω::SharedArray{Complex{Float64},2}, Nq::Int64, mP::ModelParameters, sP::SimulationParameters)
...
function calc_χ_trilex_int(Γr::SharedArray{Complex{Float64},3}, bubble::SharedArray{Complex{Float64},3}, 
                       qMultiplicity::Array{<:Real,1}, U::Float64, β::Float64; tc::Bool=false, fullRange::Bool=false)
    ...
    return NonLocalQuantities(χ, χ_ω, γ, usable)
end

function calc_DΓA_Σ_int(nlQ_sp, nlQ_ch, bubble, Gνω, FUpDo, qIndices, ωindices, usable_ν, Nk, mP::ModelParameters, sP::SimulationParameters, tc::Bool)
    ...
    transform = (length(qIndices) == 1) ? identity :  reduce_kGrid ∘ ifft_cut_mirror ∘ ifft ∘ (x->reshape(x, gridShape...))
