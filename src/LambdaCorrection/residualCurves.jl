...
"""
    find_lin_interp_root(c2_data::Array{Float64,2})
...
"""
function find_lin_interp_root(c2_data::Matrix{Float64})
...
function residuals(N_Evals::Int, 
    ...
    for i in 1:N_Evals
        ...
        if i > 2
            residuals = real.(cat(residuals, [λm_i, λd_i, lhs_c1, rhs_c1, lhs_c2, rhs_c2, μ, conv], dims=2))
        else
            residuals[:,i] = [λm_i, λd_i, lhs_c1, rhs_c1, lhs_c2, rhs_c2, μ, conv]
        end
        #println("checksums after: m =$(sum(χ_m)) d =$(sum(χ_d))")
