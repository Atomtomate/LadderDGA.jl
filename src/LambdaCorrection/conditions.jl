...
function cond_both_int(λch_i::Float64, 
    ...
    lhs_c1, lhs_c2 = lhs_int(χ_sp.data, χ_ch.data, χ_tail, kG.kMult, k_norm, χ_sp.tail_c[3], mP.β)

    #TODO: the next line is expensive: Optimize G_from_Σ
    μnew, GLoc_new, _, _ = G_from_Σladder(Σ_ladder[:,0:νmax], Σ_loc, kG, mP, sP)
...
function cond_both_sc_int(λch_i::Float64, 
    ...
    μnew, GLoc_old, _, gLoc_rfft_new = G_from_Σladder(Σ_ladder[:,0:νmax], Σ_loc, kG, mP, sP)
    ...
    while !converged
        ...
        μnew, GLoc_new, _, gLoc_rfft = G_from_Σladder(Σ_ladder[:,0:νmax], Σ_loc, kG, mP, sP; fix_n=true)
        if isnan(μnew)
            χ_λ!(χ_sp, χ_sp, -λsp_i)
            χ_λ!(χ_ch, χ_ch, -λch_i)
            break
...
function cond_both_int!(F::Vector{Float64}, λ::Vector{Float64}, 
    ...
    lhs_c1, lhs_c2 = lhs_int(χ_sp, χ_ch, χ_tail, kG.kMult, k_norm, χ_sp.tail_c[3], mP.β)
