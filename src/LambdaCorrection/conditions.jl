...
function λdm_correction_old(
    ...
    G_ladder::OffsetMatrix{ComplexF64,Matrix{ComplexF64}} =
        OffsetArray(Matrix{ComplexF64}(undef, length(kG.kMult), length(fft_νGrid)), 1:length(kG.kMult), fft_νGrid)
    Σ_ladder_work::OffsetMatrix{ComplexF64,Matrix{ComplexF64}} =
...
function run_sc_old(
    ...
    G_ladder::OffsetMatrix{ComplexF64,Matrix{ComplexF64}} =
        OffsetArray(Matrix{ComplexF64}(undef, length(kG.kMult), length(fft_νGrid)), 1:length(kG.kMult), fft_νGrid)
    Σ_ladder_old::OffsetMatrix{ComplexF64,Matrix{ComplexF64}} =
        OffsetArray(Matrix{ComplexF64}(undef, length(kG.kMult), length(νGrid)), 1:length(kG.kMult), νGrid)
    Σ_ladder::OffsetMatrix{ComplexF64,Matrix{ComplexF64}} =
        OffsetArray(Matrix{ComplexF64}(undef, length(kG.kMult), length(νGrid)), 1:length(kG.kMult), νGrid)
    E_pot = Inf
...
function run_sc_par_old(
    ...
    E_kin, E_pot, μnew, λm, lhs_c1, E_pot_2, converged = run_sc_old!(
        G_ladder,
        Σ_ladder_work,
        Σ_ladder,
        gLoc_rfft_init,
...
function run_sc_old!(
    ...
    Σ_ladder_work::OffsetMatrix,
    Σ_ladder::OffsetMatrix,
    gLoc_rfft::GνqT,
    νGrid::AbstractVector{Int},
    iωn_f::Vector{ComplexF64},
    iωn_m2::Vector{ComplexF64},
    χ_m::χT,
    χ_d::χT,
    Σ_loc::OffsetVector{ComplexF64},
    λd::Float64,
