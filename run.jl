...
function run_sim(; cfg_file=nothing)
    ...
    modelParams, simParams, env, qGrid_loc, kGrid, qGrid, νGrid, sumHelper_f, impQ_sp, impQ_ch, GImp_fft, GLoc_fft, Σ_loc_pos, FUpDo, gImp = setup_LDGA(cfg_file, false);
        
    @info "Calculating local quantities: "
    bubbleLoc = calc_bubble(νGrid, GImp_fft, nothing, modelParams, simParams)
    locQ_sp = calc_χ_trilex(impQ_sp.Γ, bubbleLoc, nothing, νGrid, sumHelper_f, modelParams.U, modelParams, simParams);
    locQ_ch = calc_χ_trilex(impQ_ch.Γ, bubbleLoc, nothing, νGrid, sumHelper_f, -modelParams.U, modelParams, simParams);

    Σ_ladderLoc = calc_Σ(locQ_sp, locQ_ch, bubbleLoc, GImp_fft, FUpDo,
                         nothing, sumHelper_f, modelParams, simParams)
